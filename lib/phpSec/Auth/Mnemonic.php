...
/**
 * Creates random passphrases (or hexadecimal uids) of specified strength which
 * you can actually remember.
 *
 * Based on mnemonic.js by Yiorgis Gozadinos
 *
 * @see https://github.com/ggozad/mnemonic.js#mnemonicjs *
 * @package phpSec
 */
class Mnemonic {
  ...
  /**
   * Get a passphrase from a secret/password.
   *
   * @param string $secret
   *   A secret/password formatted as an hexadecimal string to create
   *   passphrase from.
   */
  public function getPassPhrase($secret) {
    $passPhrase = '';

    $chunks = explode('-',chunk_split($secret, 8, '-'));
    for($i = 0; $i <= sizeof($chunks); $i++) {
      ...
      $x = hexdec($chunks[$i]);
      $w[$i][1] = $x % $this->n;
      $w[$i][2] = ($x / $this->n + $w[$i][1]) % $this->n;
      $w[$i][3] = ($x / pow($this->n, 2) + $w[$i][2]) % $this->n;

      $passPhrase .= $this->dictionary[$w[$i][1]].' '.
                     $this->dictionary[$w[$i][2]].' '.
                     $this->dictionary[$w[$i][3]].' ';
  ...
  /**
   * Get a secret/password from a passphrase.
   *
   * @param string $words
   *   List of words. The passphrase.
   *
   * @returns string
   *   Returns an hexadecimal string of the secret/password from the
   *   list of words,
   */
  public function getSecret($words) {
    $words = explode(' ', $words);
    $w = array();
    $x = '';
    foreach($words as $word) {
      if(trim($word) !== '') {
        list($key) = array_keys($this->dictionary, $word);
        $w[] = $key;
      }
      if(sizeof($w) === 3) {
        $xI = $w[0] + $this->n * $this->_mod($w[1] - $w[0], $this->n) + pow($this->n, 2) * $this->_mod($w[2] - $w[1], $this->n);
        $hexI = dechex($xI);
        if(strlen($hexI) === 7) {
          $x .= '0'.$hexI;
        } else {
   ...
   *   Strength of the secret to create, in bits.
   *
   * @returns string
   *   Returns the new secret/password as an hexadecimal string.
   */
  public function newSecret($strength = 32) {
    $rand = $this->psl['crypt/rand'];
    return bin2hex($rand->bytes($strength/8));
  }
